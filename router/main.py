...
from schema import category as Category
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:4200"],  # Si también necesitas acceso desde el mismo dominio del backend],  # Cambia esto según sea necesario
...
@app.post("/productos/", tags=['Crear'], response_model=products.Producto,
          dependencies=[Depends(auth.read_usuarios_me)])
async def create_producto(
    ...
    print(
        f"Nombre: {nombre}, Descripción: {descripcion}, ID Marca: {idMarca}, ID Categoría: {idCategoria}, Precio: {precio}, Stock: {stock}, Activo: {activo}")
    ...
    try:
        ...
        db_producto = producto.create_producto(db=db, producto=producto_data, image_path=image_path,
...
@app.get("/productos/{producto_id}", tags=['Listar por ID'], response_model=products.Producto,
         dependencies=[Depends(auth.read_usuarios_me)])
...
@app.put("/productos/{producto_id}", tags=['Actualizar'], response_model=products.ProductoUpdate,
         dependencies=[Depends(auth.read_usuarios_me)])
async def update_producto(
    ...
    try:
        ...
        print(f"Error al actualizar producto {producto_id}: {str(e)}")
        raise HTTPException(status_code=400, detail=f"Error al actualizar producto: {str(e)}")


@app.delete('/productos/{producto_id}', tags=['Eliminar'], response_model=dict, status_code=200,
            dependencies=[Depends(auth.read_usuarios_me)])
...
@app.put("/categorias/{categoria_id}", tags=['Actualizar'], response_model=Category.CategoriaUpdate,
         dependencies=[Depends(auth.read_usuarios_me)])
async def actualizar_tarea(tarea_id: int, categoria:Category.CategoriaUpdate):
    query = "UPDATE categoria SET descripcion = :descripcion, activo = :activo WHERE IdCategoria = :IdCategoria"
    values = {"id": tarea_id, "titulo": categoria.descripcion, "activa": categoria.activo}
    await database.execute(query, values)
    return {"IdCategoria": tarea_id, **categoria.dict()}
"""
# ***************************************************************************
...
async def read_marcas(skip: int = 0, limit: int = 10, db: Session = Depends(database.get_db)):
    marcas = marca.get_marcas(db, skip=skip, limit=limit)
    return marcas


@app.get("/marca/{marca_id}", tags=['Listar por ID'], response_model=marke.Marca,
         dependencies=[Depends(auth.read_usuarios_me)])
async def read_marca(marca_id: int, db: Session = Depends(database.get_db)):
    db_marca = marca.get_marca(db, marca_id=marca_id)
    if db_marca is None:
...
@app.put("/marca/{marca_id}", tags=['Actualizar'], response_model=marke.MarcaUpdate,
         dependencies=[Depends(auth.read_usuarios_me)])
